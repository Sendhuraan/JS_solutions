...
(function() {
	...
	describe('NaN (Not a Number)', function() {
		...
		describe('Check for NaN', function() {
			...
			describe('Number.isNaN Utility', function() {
				...
				it('Fortunately, we have Number.isNaN() utility (added in ES6 as polyfill), which checks for NaN without converting it to a number');
			...
			describe('Object.is Utility', function() {
				...
				it('Another way to check for NaN is to check with Object.is Utility');
	...
	describe('-0 (Negative Zero)', function() {
		...
		it('The only way to get a negative zero (-0) would be to multiply or divide (0) by unary negate (-) operator. But we could not check for (-0) using the === operator, as we would see in the following examples');

		it('We could check by doing (1 / (-0)) which should result in -Infinity');

		it('We could check by using Object.is utility');
		...
		describe('Check for (-0)', function() {
			...
			describe('Object.is Utility', function() {
				...
				it('Object.is Utility is like quadruple equals. It can be used to check -0 & NaN');
